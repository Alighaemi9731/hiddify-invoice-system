"""Public codes: 8-digit, deterministic, unique (bijective), non-sequential."""
...
def test_payment_code_8_digits_unique_and_distinct_from_invoice():
    for i in (1, 2, 99, 12345, 90_000_000 - 1):
        code = payment_code(i)
        assert code.isdigit() and len(code) == 8
        assert payment_code(i) == code  # stable
    codes = [payment_code(i) for i in range(1, 20_001)]
    assert len(set(codes)) == len(codes)  # no collisions
    # A payment and an invoice with the SAME numeric id must NOT share a code.
    assert payment_code(1) != invoice_code(1)
    assert payment_code(42) != invoice_code(42)
