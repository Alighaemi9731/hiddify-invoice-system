...
def _warn_text(items: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        "",
        "نمایندهٔ گرامی، چند کاربر با حجمِ بسیار بالا در پنلِ شما ساخته شده‌اند که در صورت عدم اصلاح، "
        "فاکتورِ این ماهِ شما را سنگین می‌کنند:",
        "─────────────────",
    ]
    for it in items:
        ...
        lines.append(f"‏👤 {name} — حجم: {gb} گیگابایت ≈ {toman} تومان")
        if it["creator_is_sub"]:
            lines.append(f"‏   ↳ ساختهٔ زیرمجموعهٔ شما: {_iso(_html.escape(it['creator_name']))}")
        if it.get("from_deleted"):
            lines.append("‏   ↳ این کاربر حذف شده اما چون مصرفِ واقعی داشته، کلِ حجمش محاسبه می‌شود.")
    lines.append("─────────────────")
    lines.append("")
    lines.append(
        "ℹ️ فاکتور بر اساسِ «حجمِ فروخته‌شدهٔ» هر کاربر محاسبه می‌شود، نه مصرف. حجمِ پیش‌فرضِ "
        "ساختِ کاربر در پنل ۱۰۰۰ گیگابایت است؛ اگر اشتباه ساخته‌اید، هم‌اکنون حجمِ کاربر را اصلاح "
        "کنید تا از سنگین‌شدنِ فاکتور جلوگیری شود."
    )
    return rtl("\n".join(lines))
