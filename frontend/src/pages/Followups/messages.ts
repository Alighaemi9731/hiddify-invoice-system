...
اگر مبلغ فاکتور برایتان محل سؤال است یا برای تسویه به مهلت نیاز دارید، لطفاً همین امروز خبر بدهید تا با هم هماهنگ کنیم و حساب‌تان محدود نشود. 🙏`,
  ...
  unregistered: `حساب نمایندگی شما هنوز به ربات ما وصل نشده است؛ به همین دلیل فاکتور دوره‌ای، یادآوری‌ها و اطلاع‌رسانی‌ها به دستتان نمی‌رسد.

اتصال یک‌بار برای همیشه انجام می‌شود و کمتر از یک دقیقه وقت می‌گیرد:
۱) وارد پنل خودتان شوید و آدرس کامل پنل را از نوار مرورگر کپی کنید (همان لینکی که با آن وارد می‌شوید).
۲) ربات را باز کنید و دکمهٔ شروع را بزنید.
۳) همان لینک را داخل ربات بفرستید.

بعد از آن، فاکتور هر ماه، مانده‌حساب و راه‌های پرداخت را مستقیم در ربات می‌بینید و دیگر لازم نیست منتظر پیام دستی بمانید.

لینک ربات را همین‌جا برایتان می‌فرستم؛ اگر جایی گیر کردید بگویید تا با هم انجامش دهیم. 🙏`,
...
export const SEGMENT_MESSAGE_HINTS: Record<CrmSegment, string> = {
  ...
  unregistered: "دعوت به اتصال: سه قدم ثبت لینک پنل در ربات. لینک ربات را جدا بفرستید.",
