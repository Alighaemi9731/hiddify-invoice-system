...
- [x] **M11** Backup now carries `secret_key` in meta.json (restore writes it to `.env` first) so a cross-server restore can decrypt the encrypted settings — restore is self-sufficient. New "reset admin passwords" button (Panels tab) → `POST /api/ops/reset-admin-passwords` → `admin_passwords.reset_admin_passwords` PATCHes each admin via `AdminApiClient.set_admin_password` (value = `admin_reset_password` setting, default "123"), reported per-admin. (Hiddify build must accept `password` on admin_user; admins otherwise authenticate by UUID.)
